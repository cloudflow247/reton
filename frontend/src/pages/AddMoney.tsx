...
export function AddMoney() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const { data: res } = await api.post('/deposits', {
        wallet_id: wallet.id,
        amount: toMinor(amount),
      })
      setDeposit(res.data as Deposit)
    } catch (err) {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-lg">
        <Card className="space-y-5">
          ...
          <Button variant="ghost" className="w-full" onClick={() => setDeposit(null)}>
    ...
    <div className="mx-auto max-w-lg">
      <Card className="space-y-6">
        ...
        <form onSubmit={submit} className="space-y-5">
          ...
          <Button type="submit" loading={loading} disabled={toMinor(amount) <= 0} className="w-full">
            Generate account
          </Button>
        </form>
