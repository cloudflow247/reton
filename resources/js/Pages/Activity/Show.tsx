...
export default function ActivityShow() {
  ...
  function savePdf() {
    setBusy('pdf')
    setStatus(null)
    // Browser print → “Save as PDF”. Receipt print CSS hides chrome.
    window.print()
    setBusy(null)
  ...
  return (
    <Page className="!px-3 sm:!px-4">
      ...
      >
        ...
        <header className="relative overflow-hidden bg-gradient-to-br from-[#0a6a4d] via-[#0e7e5c] to-[#094f39] px-5 pb-10 pt-5 text-white sm:px-6 sm:pt-6">
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-sm text-white/80">{title}</p>
            </div>
            <span
        ...
        <div className="relative -mt-5 space-y-5 px-5 pb-6 pt-1 sm:px-6">
          <div className="rounded-2xl border border-mint/15 bg-surface px-4 py-4 shadow-sm sm:px-5">
            ...
            <p className="mt-2 text-xs text-muted">{shortDate(entry.created_at)}</p>
          </div>
          ...
          {transfer?.type === 'protected' && (
            <div className="flex items-start gap-2 rounded-2xl border border-mint/20 bg-mint/[0.06] px-3.5 py-3 text-xs leading-relaxed text-muted">
          ...
          <footer className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-mint/[0.08] via-surface-2 to-mint/[0.08] px-4 py-3">
            ...
            <p className="relative text-center text-[11px] font-medium text-muted">
...
function Detail({
  ...
  return (
    <div className="rounded-2xl border border-line/80 bg-surface-2/50 px-3.5 py-3">
      ...
      <dd className={`mt-1 break-all text-sm font-semibold text-text ${mono ? 'font-num tracking-wide' : ''}`}>
        {value ?? '—'}
      </dd>
    </div>
  )
...
function WaveLayer({ className = '', muted = false }: { className?: string; muted?: boolean }) {
  const a = muted ? 'rgba(11,122,87,0.14)' : 'rgba(255,255,255,0.22)'
  const b = muted ? 'rgba(11,122,87,0.10)' : 'rgba(255,255,255,0.14)'
  const c = muted ? 'rgba(9,95,68,0.12)' : 'rgba(255,255,255,0.08)'

  return (
    <svg className={className} viewBox="0 0 600 80" preserveAspectRatio="none" aria-hidden>
      <path fill={a} d="M0 40 Q75 10 150 40 T300 40 T450 40 T600 40 V80 H0 Z" />
      <path fill={b} d="M0 52 Q100 28 200 52 T400 52 T600 52 V80 H0 Z" />
      <path fill={c} d="M0 62 Q80 48 160 62 T320 62 T480 62 T600 62 V80 H0 Z" />
    </svg>
  )
}
